...
function(input, output, session) {
  #Test outputs
  #output$value <- renderPrint({ user_input$pergene_text })
  #output$value2 <- renderPrint({ input$background_gene_with_n })
  #output$value3 <- renderPrint({ input$query_gene_n })
  #output$value4 <- renderPrint({ input$query_gene_with_n })
  ...
  output$gene_lists <- renderUI({
    ...
    observeEvent(input$create_gene_list_wa_selected_button, {
      ...
      user_input$from_enrich_selected = TRUE
      if (is.null(modifyData())){
      }else{
        do = createGeneList()
      }
    ...
    observeEvent(input$create_gene_list_ortholog_selected_button, {
      ...
      user_input$from_ortholog_selected = TRUE
      if (is.null(modifyData())){
      }else{
        do = createGeneList()
      }
    ...
    observeEvent(input$create_gene_list_wa_ipd_button, {
      ...
      user_input$from_enrich_ipd = TRUE
      if (is.null(modifyData())){
      }else{
        do = createGeneList()
      }
    ...
    observeEvent(input$create_gene_list_wa_ipd_selected_button, {
      ...
      user_input$from_enrich_selected_ipd = TRUE
      if (is.null(modifyData())){
      }else{
        do = createGeneList()
      }
    ...
    # #Reset the user_input calls for create_gene_list to FALSE
    # resetGeneListUserInputVariables <- reactive({
    #   user_input$from_enrich = FALSE
    #   user_input$from_enrich_selected = FALSE
    #   user_input$from_enrich_ipd = FALSE
    #   user_input$from_enrich_selected_ipd = FALSE
  ...
  output$graphs <- renderUI({
    ...
    if (is.null(modifyData())){ #Prior to file loading
      ...
      out[[10]] = (HTML(paste0("<h4>Clustering</h4>")))
      out[[11]] = tags$div(title=paste0("Click to show/hide the hierarchical clustering by log2foldchange of significantly DE gene in currently chosen data"),
                           actionButton("show_H_Clust","Show hierarchical clustering"))
      out[[12]] = uiOutput("h_clust")     
      
      out[[13]] = HTML("<br>")
      
      out[[14]] = (HTML(paste0("<h4>Heatmap</h4>")))
      out[[15]] = tags$div(title=paste0("Click to show/hide the heatmap of gene expression in currently chosen data"),
                           actionButton("show_filter_heatmap","Show heatmap"))
      out[[16]] = HTML("<br>")
      out[[17]] = uiOutput("filter_heatmap")     
  ...
  output$analysis <- renderUI({
    ...
    if (is.null(modifyData())){ #Prior to file loading
      ...
      out[[2]] = textOutput("enrichment_text")
      out[[3]] = textOutput("enrichment_text2")
      out[[4]] = textOutput("enrichment_text3")
      out[[5]] = HTML("<br>")
    ...
    #Returns: vector of currently selected primary gene ids 
    #Requires reactive functions: filteredData(), 
    #Requires reactive variables: user_input$species
    ...
    #GO enrichment
    #Given: Number of time button clicked
    #Returns: GO enrichment table
    #Requires UI elements: 
    #Requires reactive functions: showGOenrichment()
    output$GO_enrichment <- renderUI({
      n_clicks = showGOenrichment() #records number of times button clicked
      if (n_clicks > 0){
        if (n_clicks %% 2 == 0){
          return(NULL)
        }else{
          if (is.null(dataTableOutput("go_enrichment_table"))){
            out = list(textOutput("no_enrichment_text"))
          }else{
            out = list(dataTableOutput("go_enrichment_table"))
          }
          return(out)
        }
      }else{
      ...
      #Functions to control show/hide button  
      #Given: Clicks from input$show_go_enrichment
      #Returns: The number of clicks on input$show_go_enrichment
      #Requires reactive variables: input$show_go_enrichment
      showGOenrichment <- eventReactive(input$show_go_enrichment, {
        return(input$show_go_enrichment)
      })  
      #Given: The number of clicks on button
      #Returns: Updates the name on the button
      #Requires reactive functions: showGOenrichment()
      #Requires reactive variables: input$show_go_enrichment
      observe({ #Updates label depending on whether homology shown or not
        req(input$show_go_enrichment)
        n_clicks = showGOenrichment() #records number of times button clicked
        if (n_clicks > 0){
          if (n_clicks %% 2 == 0){
            updateActionButton(session, "show_go_enrichment",
                               label = "Show GO enrichment")
          }else{
            updateActionButton(session, "show_go_enrichment",
                               label = "Hide GO enrichment")        
          }
        }
      })   
      #Given: Gene list
      #Returns: matrix of GO terms enriched for those genes
      #Requires reactive functions: modifyData()
      #Requires reactive variables: user_input$species
      #Requires functions: speciesNames()
      ...
      #Requires reactive variables: user_input$species
      #Requires functions:speciesNames(), formatLookup()
      make_go_enrichment_table <- reactive ({
        if (is.null(modifyData())){
          return(NULL)
        }else{
          data.object = filteredData()
          ...
          data = go_enrichment(gene_names,user_input$species)
          return(data)
        }
      })
      output$go_enrichment_table <- DT::renderDataTable({
        data = make_go_enrichment_table()
    ...
    output$Publication_enrichment <- renderUI({
      ...
      make_publication_enrichment_table <- reactive({
        if (is.null(modifyData())){
          ...
          gene_names = return_primary_gene_ID()
          species_data = speciesNames(user_input$species)
    ...
    output$gProfile_enrichment <- renderUI({
      ...
      #create a new gene list from selected data - observes are with UI
  ...
  output$orthologs <- renderUI({
    if (is.null(filteredData())){ #Prior to file loading
      return(NULL)
    }else{
      ...
      out = list(tags$div(title=paste0("Select a species to find orthologs in."),
                          selectInput("select_orth_org", label = ("Orthologs in:"), 
                                      choices = names, 
                                      selected = 1)))
      out[[2]] = (dataTableOutput("ortholog_table"))
      out[[3]] = uiOutput("ortholog_enrichment_list_buttons")
  ...
  createGeneList <- reactive({
    if (is.null(modifyData())){
      ...
      if (!user_input$from_enrich){
        if(!user_input$from_enrich_selected){
          
          if (!user_input$from_enrich_ipd){
            if(!user_input$from_enrich_selected_ipd){
              
              if (!user_input$from_ortholog){
                if(!user_input$from_ortholog_selected){
                  data = data.object$filtered_data
                  gene_names = (data[gene_id_name][[1]])
      ...
      metadata = list(ui$species, length(gene_names),input$file$name,ui$select_comp,ui$s1_fpkm_num,ui$s2_fpkm_num,
                      ui$log2FC_num,ui$sig_checkbox,select2InputFoundIn,select2InputNotFoundIn,user_input$select_gene_list,description) #ui$select2InputFoundIn + ui$select2InputNotFoundIn not returning anything
      #FC mod
  ...
  #Requires reactive variables: data_cache$gene_lists, data_cache$gene_lists_metadata
  #Requires reactive variables: data_cache$gene_lists_DT
  make_gene_list_table <- reactive ({
    ...
    if (is.null(gene_lists)){
      ...
      colnames(data) = c("List name","Species","n genes","source file name","Comparison","Sample 1 Exp >=","Sample 2 Exp >=",
                         "Fold change threshold","Sig. DE only","In other comparisons","Not in other comparisons","Gene list filter","Description")
  ...
  output$gene_list_table<-DT::renderDataTable({
    ...
    #            <button type="button" class="btn btn-secondary modify"id=modify_',1:nrow(data_cache$gene_lists_DT),'>Modify</button>
    #            </div>
    #            
    #            ')
    # 
    # DT = cbind(a,DT)
    # colnames(DT)[1] = "Actions"
    datatable(DT,escape=F,rownames = FALSE,extensions = 'Buttons', options = list(dom = 'Bfrtip', buttons = I('colvis'), scrollX = TRUE))
  })
  #<button type="button" class="btn btn-secondary modify"id=save_',1:nrow(data_cache$gene_lists_DT),'>Save</button>
  ...
  observeEvent(input$Modify_row_head,{
    # if (input$lastClickId%like%"save")
    # {
    #   #row_to_del=as.numeric(gsub("save_","",input$lastClickId))
    #   #data_cache$gene_lists_DT=data_cache$gene_lists_DT[-row_to_del]
    # }
  ...
  observeEvent(input$Del_row_head,{
    ...
    #row_to_del = input$Main_table_rows_selected
  ...
  output$row_modif<-renderDataTable({
    ...
    selected_row = input$gene_list_table_rows_selected #as.numeric(gsub("modify_","",input$lastClickId))
  ...
  observeEvent(input$newValue,{
    ...
    #metadata$gene_list_name = as.character(DF[,1])
    
    metadata[[12]] = as.character(DF[,2])
    #data_cache$test2 = metadata
    data_cache$gene_lists_metadata[[selected_row]] = metadata
    
    #add back in the other columns
    old_row=data_cache$gene_lists_DT[selected_row]
    old_row = old_row[,2:12]
    DF=cbind(DF[,1],old_row,DF[,2])
    
    colnames(DF)=colnames(data_cache$gene_lists_DT)
    data_cache$gene_lists_DT[selected_row]<-DF
    
    #data_cache$test = selected_row
    
    #DF=cbind(DF[,2:dim(DF)[2]],DF[,1])
    #data_cache$gene_lists_metadata[[selected_row]] = DF
  ...
  #Gene list genes tab
  #Given: table from output$table2
  #Returns: this as a datatable
  ...
  output$gene_list_genes_table <- renderUI({
    ...
    filterFileData <- reactive({
      ...
      out = list(table = out_table, unused = unused_genes)
      return(out)
    }) 
    
    # Display gene list genes  
    #Given: table from filterFileData()
    #Returns: this as a datatable
    #Requires reactive functions: filterFileData()
    #Requires reactive variables: data_cache$lookup, user_input$select_gene_list2
    ...
    options = list(pageLength = 25, order = list(list(2, 'desc'), list(1, 'asc'))
      ...
      observeEvent(input$table2_rows_selected,{
        if (is.null(input$table2_rows_selected)){
          ...
          if (length(input$table2_rows_selected) > 0){
            user_input$table2_rows_selected = input$table2_rows_selected
          }
          format = user_input$format
          format_details = formatLookup(format)
          gene_id_position = format_details$gene_id_position
          ...
          data = filterFileData()
          table = data$table 
          row = user_input$table2_rows_selected
  ...
  sampleNames <- reactive({
    if (is.null(rawData())){ #Prior to loading a dataset
      ...
      if (!is.na(condition_1_position)){
        ...
        conditions = unique(c(levels(raw_gene_names[condition_1_name][[1]]),levels(raw_gene_names[condition_2_name][[1]])))
      }else{
  ...
  plot_plot <- function(){
    ...
    if(user_input$volcano_plot_no_rearrange | user_input$volcano_plot_grey_data){ #using min max from original data
      ...
      if (length(data2[,FC_position]) > 0){
        x_min = min(data2[,FC_position])
        x_max = max(data2[,FC_position])
      }else{
        x_min = min(data[,FC_position])
        x_max = max(data[,FC_position])
      }
      y_min = 0
